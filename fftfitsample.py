...
#FT the fake data by hand and solving for positions and differences of maxima in different detectors

#datr=np.loadtxt('tsample2.dat')
...
t=dat1[:,1]
detind=dat1[:,0]
alldets=np.array([11,12,13,14,16,17]) #The detectors we're interested in
...
detinds=np.linspace(0,5,6) #indices of the detectors we care about
...
for i in range(0,len(detinds)):
	...
	pars=np.append(pars,alldets[i]-9) #write
	...
	pars=np.append(pars,alldets[i]-9)
...
phas=[np.angle(pft)]
parmat2=np.concatenate([parmat.T,phas],0).T
...
#np.savetxt('fftfitsamplepars2.dat',parmat2,fmt='%01d %01d %+.5e %+.5e %+.5e %+.5e %+.5e %+1.6f %1.6f')
np.savetxt('fftfitsampleparsnp.dat',parmat2,fmt='%01d %01d %+.5e %+.5e %+.5e %+.5e %+.5e %+1.6f %1.6f')
...
#np.savetxt('fftfitsampledifpars2.dat',pardif,fmt='%01d %01d %+1.6f %+1.3f')
np.savetxt('fftfitsampledifparsnp.dat',pardif,fmt='%01d %01d %+1.6f %+1.3f')
